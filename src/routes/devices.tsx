...
import { devices, petAvatar } from '../lib/petguard-data'
import { addCollar, findDevice } from '../lib/services/devices'
...
function DeviceCard({ device }: { device: typeof devices[number] }) {
  const onFind = useCallback(() => findDevice(device.serial), [device.serial])
  ...
  return (
    <article className="card p-5">
      ...
      <ActionButton
        fn={onFind}
        label={`Find device ${device.serial}`}
        doneLabel="Signal sent!"
        className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 px-4 py-3 text-sm font-bold text-white hover:bg-slate-800"
      >
...
function AddCollarButton() {
  const onAdd = useCallback(() => addCollar('New collar'), [])
  return (
    <ActionButton
      fn={onAdd}
      label="Add new collar"
      doneLabel="Collar added!"
      className="inline-flex items-center gap-2 rounded-full bg-teal-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-teal-600/20 hover:bg-teal-700"
    >
