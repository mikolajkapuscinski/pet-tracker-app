      ...
      <section className="grid gap-6 xl:grid-cols-[1.45fr_0.85fr]">
        <article className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <button className="rounded-full bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-200">
              Manage plan
            </button>
          </div>
        ...
        <aside className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <button className="mt-4 w-full rounded-2xl border-2 border-dashed border-slate-200 px-4 py-3 text-sm font-semibold text-slate-500 transition hover:bg-slate-50">
            Update card
          </button>
        </aside>
      </section>
      ...
      <section className="mt-6 overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm">
        ...
        <div className="divide-y divide-slate-100">
          {billingHistory.map((entry) => (
            <div key={entry.date + entry.amount} className="grid gap-4 px-6 py-4 sm:grid-cols-[1fr_auto_auto] sm:items-center">
              <div>
